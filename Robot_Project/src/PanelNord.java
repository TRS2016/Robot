...
public class PanelNord {
	...
	static JButton NombrepapierGras = new JButton();
	static int papierGras ;
	...
	public void seTPanelNord(){
		...
		gc.gridy = 1;
		gc.gridwidth = 4;
		NombrepapierGras.setText("Nombre de papier gras : "+String.valueOf(papierGras));
		nord.add(NombrepapierGras, gc); //On rajoute le bouton Papier gras au panel nord
	...
	public void MAS(Monde m){
		...
		for(int i=0; i<10; i++)
			for(int j=0; j<10; j++){
				...
				papierGras = m.comptePapierGras();
				NombrepapierGras.setText("Nombre de papier gras : "+papierGras);
				NombrepapierGras.validate();
				NombrepapierGras.repaint();
			}
	}
}
